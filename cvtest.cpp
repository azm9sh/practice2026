...
void encode(int run, int val, const unsigned int* t_cwd,
  ...
  int s = 0;
  int uval = (val < 0) ? -val : val;  // uval は unsigned val
  int bound = 1;                      // boundはカウンタ
  while (uval >= bound) {
    bound += bound;
    s++;
  }
  if (X == 0) {
    ...
    // AC
    // t_cwd[(run << 4) + s] と t_len[(run << 4) + s]
    // ビットの値としてビットストリームに書き込む -> 4bitのシステムを書き込む
...
int main(int argc, char* argv[]) {
  ...
  std::cout << "codestream zize = " << length << std::endl;
